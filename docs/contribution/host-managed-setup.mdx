...
If you use fish, run `source venv/bin/activate.fish` instead. You can also skip this step entirely because `bash scripts/start_services_dev.sh` activates the venv itself.
...
```
	Keep this running before the backend script; it provides the PostgreSQL, Redis, and MinIO endpoints that the API expects.
