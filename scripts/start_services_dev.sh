...
wait_for_tcp_endpoint() {
  ...
  python - "$label" "$connection_string" "$timeout_seconds" <<'PY'
import socket
import sys
import time
...
if port is None:
    if parsed.scheme == "postgresql+asyncpg":
        ...
        raise SystemExit(f"{label}: could not determine a port from {raw_value!r}")
...
print(
    f"{label} was not reachable at {host}:{port} after {timeout_seconds}s. "
...
if [[ -n "${DATABASE_URL:-}" ]]; then
  echo "Checking PostgreSQL at ${DATABASE_URL} ..."
  wait_for_tcp_endpoint "PostgreSQL" "$DATABASE_URL" "$DEPENDENCY_CHECK_TIMEOUT"
fi

if [[ -n "${REDIS_URL:-}" ]]; then
  echo "Checking Redis at ${REDIS_URL} ..."
  wait_for_tcp_endpoint "Redis" "$REDIS_URL" "$DEPENDENCY_CHECK_TIMEOUT"
fi

if [[ -n "${MINIO_ENDPOINT:-}" ]]; then
  minio_endpoint="$MINIO_ENDPOINT"
  if [[ -z "${minio_endpoint:-}" && -n "${MINIO_PUBLIC_ENDPOINT:-}" ]]; then
    minio_endpoint="$MINIO_PUBLIC_ENDPOINT"
  fi
  if [[ -z "${minio_endpoint:-}" ]]; then
    minio_endpoint="http://localhost:9000"
  fi
  echo "Checking MinIO at ${minio_endpoint} ..."
  wait_for_tcp_endpoint "MinIO" "$minio_endpoint" "$DEPENDENCY_CHECK_TIMEOUT"
fi
