...
async def signup(request: SignupRequest):
    # In local OSS mode, treat an existing email as a password reset/rebind.
    # This keeps local contributor accounts recoverable when a password is lost
    # or a row was created before the current UI flow.
    existing_user = await db_client.get_user_by_email(request.email)
    if existing_user and AUTH_PROVIDER != "local":
        raise HTTPException(status_code=409, detail="Email already registered")

    hashed = hash_password(request.password)
    if existing_user:
        await db_client.update_user_password_hash(existing_user.id, hashed)
        user = await db_client.get_user_by_id(existing_user.id)
        if user is None:
            raise HTTPException(status_code=500, detail="Failed to reload user")
    else:
