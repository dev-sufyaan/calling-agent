...
function normalizeLocalhostUrl(rawUrl: string) {
    try {
        ...
        // Not a valid URL — return it unchanged and let the caller handle it.
    }
...
export function getServerBackendUrl() {
    return normalizeLocalhostUrl(process.env.BACKEND_URL || 'http://api:8000');
}
...
export function resolveBrowserBackendUrl(backendApiEndpoint?: string | null): string {
    return (
        ...
        (backendApiEndpoint ? normalizeLocalhostUrl(backendApiEndpoint) : null) ||
